...
def amalgamate(conn, acct_id_0: int, acct_id_1: int):
    ...
    # Fetch balances for the source account
    cursor.execute(
    ...
    total = savings_row[0] + checking_row[0]

    # Transfer total to destination account's savings and reset source checking
    cursor.execute(
...
def balance(conn, acct_id: int) -> float:
    ...
    return savings_row[0] + checking_row[0]
...
def send_payment(conn, send_acct: int, dest_acct: int, amount: float):
    ...
    # Validate sender and destination accounts exist
    cursor.execute(
        f"SELECT custid FROM {TABLENAME_ACCOUNTS} WHERE custid = %s",
        (send_acct,),
    )
